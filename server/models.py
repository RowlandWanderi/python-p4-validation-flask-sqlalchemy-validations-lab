...
class Post(db.Model):
    ...
    def validate_length(self, key, string):
        if( key == 'content'):
            if len(string) < 250:
                raise ValueError("The Post content must be greater than or equal 250 characters long.")
        if( key == 'summary'):
            if len(string) > 250:
                raise ValueError("The Post summary must be less than or equal to 250 characters long.")
        return string
